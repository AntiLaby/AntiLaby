...
public class AntiLabyPackager {
	...
	public AntiLabyPackager(Player p, HashMap<LabyModFeature, Boolean> disabledLabyModFeatures, boolean forceIgnoreBypassPermission) {
		this.p = p;
		this.disabledLabyModFeatures = disabledLabyModFeatures;
		this.forceIgnoreBypassPermission = forceIgnoreBypassPermission;
	}
		...
		if(!forceIgnoreBypassPermission) {
			...
			if(p.hasPermission("antilaby.bypass")) {
				if(AntiLaby.getInstance().getConfig().getBoolean("AntiLaby.EnableBypassWithPermission")) {
					ignorePlayer = true;
					System.out.println("[AntiLaby/INFO] Player " + p.getName() + " (" + p.getUniqueId() + ") has the permission 'antilaby.bypass' and has been ignored.");
				} else {
					ignorePlayer = false;
				}
			} else {
