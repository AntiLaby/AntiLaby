      ...
      if (Bukkit.getOnlinePlayers().contains(targetPlayer)) {
        ...
        if (sender instanceof Player) {
          ...
          if (player.hasPermission(Constants.PERMISSION_LABYINFO)) {
            if (uses) {
              player.sendMessage(Constants.PREFIX + lan.translate(prefix + "labyMod", player, targetPlayer.getName()));
            } else {
              player.sendMessage(Constants.PREFIX + lan.translate(prefix + "noLabyMod", player, targetPlayer.getName()));
            }
          } else {
            player.sendMessage(Constants.PREFIX + lan.translate("antilaby.command.noPermission", player));
            AntiLaby.LOG.debug("Player " + player.getName() + " (" + player.getUniqueId() + ") tried to use " + "LabyInfo: Permission 'antilaby.labyinfo' is " + "missing!");
          }
        } else if (uses) {
        ...
        sender.sendMessage(Constants.PREFIX + lan.translate(prefix + "playerOffline", (Player) sender, args[0]));
      } else {
