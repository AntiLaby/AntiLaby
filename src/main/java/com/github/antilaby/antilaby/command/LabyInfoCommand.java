  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (args.length == 1) {
      ...
      if (Bukkit.getOnlinePlayers().contains(targetPlayer)) {
        final boolean uses =
            IncomingPluginChannel.getLabyModPlayers().containsKey(targetPlayer.getUniqueId().toString());
        if (sender instanceof Player) {
          final Player player = (Player) sender;
          if (player.hasPermission(Constants.PERMISSION_LABYINFO)) {
            if (uses) {
              player.sendMessage(Constants.PREFIX + lan.translate(prefix + "labyMod", player,
                  targetPlayer.getName()));
            } else {
              player.sendMessage(Constants.PREFIX + lan.translate(prefix + "noLabyMod", player,
                  targetPlayer.getName()));
            }
          } else {
            player.sendMessage(Constants.PREFIX + lan.translate("antilaby.command.noPermission",
                player));
