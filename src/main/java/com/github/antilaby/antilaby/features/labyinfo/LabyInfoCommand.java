		...
		if (commandAvailable) {
			if (args.length == 1) {
				...
				if (Bukkit.getOnlinePlayers().contains(targetPlayer)) {
					...
					if (sender instanceof Player) {
						...
						if (player.hasPermission(Constants.PERMISSION_LABYINFO)) {
							if (labyPlayer.usesLabyMod())
								player.sendMessage(lan.translate("antilaby.command.labyInfo.labyMod", player,
										targetPlayer.getName()));
							else
								player.sendMessage(lan.translate("antilaby.command.labyInfo.noLabyMod", player,
										targetPlayer.getName()));
						} else {
					...
					} else if (labyPlayer.usesLabyMod())
			...
			AntiLaby.LOG.info("Sorry, but LabyInfo is currently not available.");
		return true;
	}
